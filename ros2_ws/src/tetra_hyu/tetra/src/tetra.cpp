...
public:
	TETRA() : Node("tetra")
	{
		...
		//service list///////////////////////////////////////////////////////////////////////////////////////
		parameter_read_srv = create_service<tetra_msgs::srv::ParameterRead>(
        	"param_read_cmd", 
		std::bind(&TETRA::Parameter_Read_Command, this, std::placeholders::_1, std::placeholders::_2));

		parameter_write_srv = create_service<tetra_msgs::srv::ParameterWrite>(
        	"param_write_cmd", 
		std::bind(&TETRA::Parameter_Write_Command, this, std::placeholders::_1, std::placeholders::_2));

		set_move_mode_srv = create_service<tetra_msgs::srv::SetMoveMode>(
        	"mode_change_cmd", 
		std::bind(&TETRA::Movemode_Change_Command, this, std::placeholders::_1, std::placeholders::_2));

		linear_position_move_srv = create_service<tetra_msgs::srv::LinearPositionMove>(
        	"linear_move_cmd", 
		std::bind(&TETRA::Linear_Move_Command, this, std::placeholders::_1, std::placeholders::_2));

		angular_position_move_srv = create_service<tetra_msgs::srv::AngularPositionMove>(
        	"angular_move_cmd", 
...
int main(int argc, char * argv[])
{
	...
	while (rclcpp::ok())
	{
		...
		else 
		{
			...
			dt=(tetra.current_time - tetra.last_time).seconds(); //dt=(current_time-last_time).toSec();
			for(int i=0;i<3;i++)
			{
